    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        ...
        <li class="nav-header">Menu</li>
            ...
            <li class="nav-item">
                <a href="./backlog.php" class="nav-link active">
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Listado de backlog  </h3>
                </div>
                <!-- /.card-header -->
                    ...
                    <table id="list_usuarios" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        <?php
                            if ($result = $mysqli -> query("SELECT cr.id,cr.name,cr.identification,sso.created_at,s.name as estado FROM central_risk as cr inner join scheduling_occidente as so on so.central_risk_id=cr.id inner join scheduling_status_occidente as sso on so.id=sso.scheduling_id inner join status as s on s.id=sso.status_id where cr.user_id=7 and sso.current=1 and (sso.created_at< CURDATE() and date_format(sso.created_at,'%m') < date_format(now(),'%m'))")) {
