<?php
   ...
   $mysqli = new mysqli("db","db_american_group","4m3r1c4n2021","db");
    ...
    $id_base=$_GET['id'];
     ...
     // Perform query
     if (!$mysqli -> query("UPDATE `bases_archivo` SET `is_active` = '0' WHERE `bases_archivo`.`id` = '$id_base'")) {
    ...
    if (!$mysqli -> query("UPDATE `base_$prefijo` SET `is_active` = '0' WHERE `base_$prefijo`.`archivo_id` = $id_base;")) {
        echo("Error description: " . $mysqli -> error);
    }

    $mysqli -> close();
    header('Location: ../admin/bases.php?sa=1&name_base='.$name_base.'&campana_select_id='.$id_campana);
    exit;
